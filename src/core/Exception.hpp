...
namespace durak::core::error
{
 ...
 // Fine-grained reasons; grouped by action type.
    ...
    struct RuleViolation
    {
        ...
        auto with_phase(Phase p) -> RuleViolation& { phase = p; return *this; }
        auto with_actor(PlyrIdxT s) -> RuleViolation& { actor = s; return *this; }
        auto with_attacker(PlyrIdxT s) -> RuleViolation& { attacker = s; return *this; }
        auto with_defender(PlyrIdxT s) -> RuleViolation& { defender = s; return *this; }
        auto with_cap_used(std::uint8_t v) -> RuleViolation& { capacity_used = v; return *this; }
        auto with_cap_free(std::uint8_t v) -> RuleViolation& { capacity_free = v; return *this; }
        auto with_def_hand(std::uint8_t v) -> RuleViolation& { defender_hand = v; return *this; }
        auto with_attempted(std::uint8_t v) -> RuleViolation& { attempted_count = v; return *this; }
        auto with_rank(Rank r) -> RuleViolation& { rank = r; return *this; }
    };
    ...
    inline auto describe(RuleViolation const& v) -> std::string
    {
        ...
        if (v.phase)          s += std::format(" | phase={}", (*v.phase == Phase::Attacking ? "A" : (*v.phase == Phase::Defending ? "D" : "C")));
